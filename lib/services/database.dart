...
class Database {
  ...
  Future<String> generateId() async {
    ...
    while (searching) {
      ...
      DocumentSnapshot check = await _db.collection('orders').doc(id).get();
      if (!check.exists) {
        searching = false;
      }
    }
  ...
  Future<void> createOrder(String id, Map<String, dynamic> data) async {
    try {
      ...
      _db
          ...
          .where('end_at', isLessThan: timeNow)
          .get()
          .then((value) {
        if (value.docs != null && value.docs.length > 0) {
          value.docs.forEach((element) {
            _db
                .collection("orders")
                .doc(element.id)
                .update({'status': 97, 'status_code': 'time_end'});
          });
        }
      });
    } catch (e) {
  ...
  Future<List<SparesModel>> getSparesOrders(
      {String userId, String whereF = 'userId'}) async {
    var ref =
        await _db.collection('spares').where(whereF, isEqualTo: userId).get();
    return ref.docs.map((d) => SparesModel.fromFirestore(d)).toList();
  }

  Stream<List<SparesModel>> getSparesOrdersStream(
      {String userId, String whereF = 'userId'}) {
    var ref = _db.collection('spares').where(whereF, isEqualTo: userId);
    return ref.snapshots().map(
        (list) => list.docs.map((d) => SparesModel.fromFirestore(d)).toList());
  }
  ...
  Future<SosModel> getUserSos() async {
    try {
      User _user = FirebaseAuth.instance.currentUser;
      QuerySnapshot doc = await _db
          .collection("sos")
          .where('helperId', isEqualTo: _user.uid)
          .where('status', isEqualTo: "accepted")
          .limit(1)
          .get();
      if (doc.docs != null && doc.docs.length > 0) {
        DocumentSnapshot dataDoc = doc.docs.first;
        return SosModel.fromFirestore(dataDoc);
      } else {
        return null;
      }
    } catch (e) {
  ...
  Stream<List<OrderModel>> getWaitingOrders({String userId}) {
    ...
    return ref.snapshots().map(
        (list) => list.docs.map((d) => OrderModel.fromFirestore(d)).toList());
  }

  Stream<List<OrderModel>> getWaitingOrdersForDrivers({String userId}) {
    Query query = _db.collection('orders').where('status', isEqualTo: 1);
    UserModel user = Get.find<UserController>().user;

    List vehicles =
        user.personalData != null ? user.personalData['vehicles'] ?? [] : [];
    List vehiclesInt =
        user.personalData != null ? user.personalData['vehiclesInt'] ?? [] : [];
    if (vehicles.contains("van".tr) && vehicles.contains("pickup".tr)) {
    } else if (vehicles.contains("van".tr) || vehiclesInt.contains(1)) {
      print("test van");
      query = _db
          .collection('orders')
          .where('status', isEqualTo: 1)
          .where('vehicleType', isEqualTo: 1);
    } else if (vehicles.contains("pickup".tr) || vehiclesInt.contains(2)) {
      print("test pickup");
      query = _db
          .collection('orders')
          .where('status', isEqualTo: 1)
          .where('vehicleType', isEqualTo: 2);
    }
  ...
  Future<List<UserModel>> getSosUsers() async {
    var ref =
        await _db.collection('users').where('isSos', isEqualTo: true).get();
    return ref.docs.map((d) => UserModel.fromDocumentSnapshot(d)).toList();
  }

  Stream<List<SosModel>> getSosRequests({String userId}) {
    var ref = _db.collection('sos').where("status", isEqualTo: "searching");
    return ref.snapshots().map(
        (list) => list.docs.map((d) => SosModel.fromFirestore(d)).toList());
  }
  ...
  Future<bool> setDriver(
    ...
    try {
      ...
      bool insert = true;
      if (status == 3) {
        insert = await _db.runTransaction((transaction) async {
          ...
          if (snapshot['status'] == 3) {
            Get.back();
            GetxWidgetHelpers.mSnackBar('خطاء!', 'يرجى محاولة في وقت لاحق.');
            return false;
          }
          return true;
        });
      ...
      if (status == 3) {
        ...
        await this.updateDocument(
            docName: "users",
            docID: _clientUser.id,
            data: {"onOrderID": orderID, "isOnRide": true},
        ...
        PushNotificationService().sendNotification(
            ...
            title: 'تم قبول طلبك.',
            body:
                'تم قبول طلبك من طرف سائق ${user.firstName ?? ''} سيارة رقم ${user?.personalData["plateNumber"] ?? '-'}',
        ...
        PushNotificationService().sendNotification(
            ...
            body:
                'تم شحن البضاعة من طرف ${user.firstName ?? ''} سيارة رقم ${user?.personalData["plateNumber"] ?? '-'}',
            token: _clientUser.token ?? '');
        ...
        PushNotificationService().sendNotification(
            ...
            body:
                'تم الانتهاء من نقل طلبك من طرف ${user.firstName ?? ''} سيارة رقم ${user?.personalData["plateNumber"] ?? '-'}',
            token: _clientUser.token ?? '');
        if (orderModelX.isSpareOrder == true && orderModelX.spareID != null) {
          await this.updateDocument(
              create: false,
              docName: "spares",
              docID: orderModelX.spareID,
              data: {
                "status": 9,
              },
              donTShowLoading: true);
        ...
        await this.updateDocument(
            docName: "users",
            docID: orderModelX.userId,
            data: {"onOrderID": null, "isOnRide": false},
            donTShowLoading: true);
        await this.updateDocument(
            docName: "users",
            docID: orderModelX.driverId,
            data: {"onOrderID": null, "isOnRide": false},
            donTShowLoading: true);
        if (orderModelX.payCash) {
          await PaymentController.createCommissionPayment(
              userId: orderModelX.driverId,
              order: orderModelX,
              title: 'commissionDue');
        } else {
          if (orderModelX.isSpareOrder != true) {
            await PaymentController.createOrderPayment(
                ...
                typeX: "discount",
                payVia: "app",
                title: 'cutBalanceForOrder');
          }
          await PaymentController.createOrderPayment(
              forId: orderModelX.driverId,
              order: orderModelX,
              typeX: "added",
              payVia: "app",
              cutCommission: true,
  ...
  Future<bool> setOrderStatus(
    ...
    try {
      Future.delayed(
          ...
          () => Get.dialog(Center(child: CircularProgressIndicator()),
              barrierDismissible: false));
      Map<String, dynamic> data = {'status': status, 'reasonCanceled': reason};
      DateTime now = DateTime.now();
      String jm = intl.DateFormat.yMd().add_jm().format(now);
      Map<String, dynamic> dates = order.dates;
      if (status == 4) {
      ...
      if (status == 1) {
        await this.updateDocument(
            docName: "users",
            docID: order?.userId,
            data: {"onOrderID": order?.id, "isOnRide": true},
            donTShowLoading: true);
        Get.back();
      }
      if (status == 97) {
        ...
        if (isDriver) {
          ...
          PushNotificationService().sendNotification(
              ...
              title: 'تم الغاء طلبك.',
              body:
                  'قام لتو السائق ${order.driverName} بالغاء طلبك لسبب $reason}',
              token: _clientUser.token ?? '');
        }
        if (order?.driverId != null) {
          await this.updateDocument(
              docName: "users",
              docID: order?.driverId,
              data: {"onOrderID": null, "isOnRide": false},
              donTShowLoading: true);
          if (!isDriver) {
  ...
  Stream<List<UserModel>> getShops({String userId, String brand}) {
    ...
    return ref.snapshots().map((list) =>
        list.docs.map((d) => UserModel.fromDocumentSnapshot(d)).toList());
  }
  ...
  Future<void> updateDocument(
      ...
      String docID,
      Map<String, dynamic> data,
      bool donTShowLoading = false,
      bool create = true}) async {
    print(
        "================================== updating $docName document and donTShowLoading : $donTShowLoading");
  ...
  Future<void> deleteDocument(
      {String docName, String docID, bool displayLoading = true}) async {
    print(
        "================================== deleting $docName document with displayLoading : $displayLoading");
  ...
  Future<void> deleteInsideDocument(
      {String colName,
      String colID,
      String docName,
      String docID,
      bool displayLoading = true}) async {
    print(
        "================================== deleting $docName document with displayLoading : $displayLoading");
  ...
  Future<void> updateAppInformation({String name, var theValue = 1}) async {
    DocumentSnapshot data =
        await _db.collection("appInformation").doc("overview").get();
    String fName = name;
    var value;
    if (name == "drivers") {
      fName = "drivers";
  ...
  Future<List<FloorModel>> fetchFloors() async {
    DocumentSnapshot docs =
        await _db.collection("appInformation").doc("floors").get();
    List<FloorModel> floors = <FloorModel>[];
    if (docs.exists && docs.data() != null && (docs.data() as Map).length > 0) {
      (docs.data() as Map).forEach((key, value) {
        if (value != null) {
          FloorModel _floorModel = FloorModel.fromMap(value);
          floors.add(_floorModel);
        }
      });
    }
    return floors;
  ...
  Future<List<DistanceModel>> fetchDistances() async {
    DocumentSnapshot docs =
        await _db.collection("appInformation").doc("distancesPrice").get();
    List<DistanceModel> distances = <DistanceModel>[];
    if (docs.exists && docs.data() != null && (docs.data() as Map).length > 0) {
      (docs.data() as Map).forEach((key, value) {
        if (value != null) {
          DistanceModel _distanceModel = DistanceModel.fromMap(value);
          distances.add(_distanceModel);
        }
      });
    }
    return distances;
  ...
  Future<void> sendAdminNotifications(
      {String title, String body, String iid, String type}) async {
    _db
        .collection("users")
        .where('isAdmin', isEqualTo: true)
        .get()
        .then((admins) {
      if (admins.docs != null && admins.docs.length > 0) {
