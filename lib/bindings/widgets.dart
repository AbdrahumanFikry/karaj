...
Widget appBar({String title, Color clr, Color iconClr}) {
  return AppBar(
    title: Text(title ?? '',
        style: TextStyle(color: Get.theme.colorScheme.primaryVariant)),
    centerTitle: true,
...
Widget circleAvatarNetwork(
  ...
  return InkWell(
    onTap: () async {
      if (zoomIn == true) {
        await Helpers.launchUrl(url: src);
      }
    },
    child: src != null && src.trim().isNotEmpty
        ? CircleAvatar(
            backgroundColor: color == null ? Colors.grey : color,
            backgroundImage:
                imageWidget == null ? NetworkImage(src) : imageWidget,
            radius: radius,
          )
        : CircleAvatar(
            backgroundColor: color == null ? Colors.grey : color,
    ...
    return Row(
      ...
      children: List.generate(5, (index) {
        return Icon(Icons.star,
            color: index < v ? Colors.yellow[600] : Colors.grey, size: 17.0);
      }),
    );
  }
}
...
class StarDisplayVote extends StatelessWidget {
  ...
  const StarDisplayVote({Key key, this.value, this.onChanged})
      : super(key: key);
...
Widget displayNearByKm({var km}) {
  return RichText(
    text: TextSpan(
        text: '$km ',
        style: TextStyle(
            fontFamily: 'Cairo', fontWeight: FontWeight.bold, fontSize: 13.0),
        children: <TextSpan>[
          TextSpan(
              text: "km".tr,
              style: TextStyle(
...
Widget flatButtonWidget(
    {BuildContext context, String text, VoidCallback callFunction}) {
  return TextButton(
  ...
  return TextButton(
    ...
    child: onLoading
        ? Container(
            height: 21,
            width: 21,
            child: CircularProgressIndicator(color: Get.theme.backgroundColor))
        : Text('$text',
            style: Get.theme.textTheme.headline6.copyWith(color: textColor)),
    style: TextButton.styleFrom(
      backgroundColor: colourX,
      padding:
          padding == null ? EdgeInsets.symmetric(horizontal: 18.0) : padding,
      shape: RoundedRectangleBorder(
...
InputDecoration inputDecorationUi(BuildContext context, hintText,
    {Color color,
    IconData icon,
    String suffixText = "",
    String counterText = "",
  ...
  return Container(
    ...
    child: ListTile(
      ...
      subtitle: Text(subtitle ?? '',
          style: TextStyle(
            color:
                selected ? Get.theme.colorScheme.secondaryVariant : Colors.grey,
            fontSize: 11.0,
          )),
      trailing: Text(
        "$trailing ${"sar".tr}" ?? '',
...
Widget paymentListDetails(String name, String value,
    {double size,
    String keyTr = '',
    String uid,
    Color clr,
    ...
    var lat,
    var lng}) {
  Widget vText = Text(value,
      style: TextStyle(
          fontSize: size != null ? size : null,
          color: clr != null ? clr : null),
  ...
  return Container(
    ...
    child: Row(
      ...
      children: [
        ...
        keyTr.toLowerCase().contains('phone') ||
                keyTr.toLowerCase().contains('driverName') ||
                keyTr.toLowerCase().contains('map')
            ? InkWell(
                onTap: () {
      ...
      child: Align(
        ...
        child: Image(
          image: Get.isDarkMode
              ? AssetImage('assets/images/logo-light.jpg')
...
class ContainerCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Get.theme.backgroundColor, boxShadow: [
        BoxShadow(
