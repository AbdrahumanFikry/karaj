...
class DriverPersonalInformation
    ...
    return Obx(
      () => Column(
        ...
        children: [
          ...
          _textInputInformation(
              mapKey: "nationalityExpiryDate", context: context),
          ...
          Container(
              height: 50,
              width: Get.width,
              child: _buildCitiesMultiSelected(context)),
          Padding(
            padding: const EdgeInsets.only(right: 5.0),
            child: Align(
              alignment: Alignment.centerRight,
          ...
          Container(
              height: 50,
              width: Get.width,
              child: _buildVehicleSelected(context)),
          _buildAvatarPickupButton(
              text: "uploadAvatar".tr, key: 'imageAvatarPath'),
          ...
          controller.personalData['hasLicense'] != null &&
                  controller.personalData['hasLicense'] == true
              ? _textInputInformation(
                  mapKey: "licenseExpiryDate", context: context)
              : SizedBox.shrink(),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                Expanded(
                    child: _textInputInformation(
                        mapKey: "plateLetters", context: context)),
                Expanded(
                    child: _textInputInformation(
                        mapKey: "plateNumbersX", context: context)),
              ],
            ),
          ),
          CheckboxListTile(
          ...
          _buildAvatarPickupButton(
              text: "imageSideViewCar".tr, key: 'imageSideViewCar'),
          _buildAvatarPickupButton(
              text: "imageCarFront".tr, key: 'imageCarFront'),
        ],
      ),
    );
  }

  Widget _textInputInformation(
      {String mapKey, BuildContext context, double width, double mLeft = 0.0}) {
    return Container(
      width: width,
      ...
      child: TextFormField(
        ...
        onSaved: (value) {
          ...
          if (mapKey == "plateNumbersX") {
            String plt =
                "${controller.personalData["plateLetters"]} - ${controller.personalData["plateNumbersX"]}";
            controller.personalData["plateNumbers"] =
                Helpers.englishToArabic(plt);
            controller.personalData["plateNumber"] =
                Helpers.englishToArabic(plt);
          }
        },
        validator: (value) {
        ...
        decoration: inputDecorationUi(context, mapKey.tr,
            color: Get.theme.scaffoldBackgroundColor),
        keyboardType:
            mapKey.contains('Date') ? TextInputType.number : TextInputType.text,
      ),
    );
  }
        ...
        builder: (ctrl) {
          return ListView.builder(
              ...
              itemBuilder: (BuildContext context, index) {
                ...
                return InkWell(
                  onTap: () {
                    ctrl.updateVehiclesSupport(car, index);
                  },
                  child: Container(
        ...
        child: Container(
            ...
            child: Column(
              children: [
                ...
                controller.personalData[key ?? ''] != null
                    ? Container(
                        child: Image.file(
                            File(controller.personalData[key ?? ''])),
                      )
                    : SizedBox.shrink()
              ],
            )),
