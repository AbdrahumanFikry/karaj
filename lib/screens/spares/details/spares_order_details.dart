...
class SparesOrderDetails extends GetWidget<ModifiedOrderController> {
            ...
            children: [
              ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, index) {
                    ...
                    return InkWell(
                      onTap: () async {
                        if (ctrl.orderData.status == 1 &&
                            _userAuth.uid == ctrl.orderData?.shopId) {
                              ...
                              child: ListTile(
                                ...
                                title: Row(
                                  ...
                                  children: [
                                    ...
                                    ctrl.orderData?.userId == _userAuth.uid &&
                                            ctrl.orderData?.status == 2
                                        ? IconButton(
              ...
              Container(
                ...
                child: ctrl.orderData.status >= 1
                    ? Column(
                        children: [
                          ctrl.orderData?.shopId ==
                                  FirebaseAuth.instance.currentUser?.uid
                              ? Column(
                                  children: [
                                    paymentListDetails('clientName'.tr,
                                        "${ctrl.orderData.userName}" ?? ''),
                                    paymentListDetails('clientPhone'.tr,
                                        "${ctrl.orderData.userPhone}" ?? '',
                                        keyTr: 'clientPhone',
                                        clr: Colors.blueAccent),
                                  ],
                                )
                              : ctrl.orderData.payed == true
                                  ? Column(
                                      children: [
                                        paymentListDetails('shopName'.tr,
                                            "${ctrl.orderData.shopName}" ?? ''),
                                        paymentListDetails('numberPhone'.tr,
                                            "${ctrl.orderData.shopPhone}" ?? '',
                                            keyTr: 'phone',
                                  ...
                                  : SizedBox.shrink(),
                          paymentListDetails(
                              'brand'.tr, "${ctrl.orderData?.brand}"),
                          paymentListDetails(
                              'category'.tr, "${ctrl.orderData?.category}"),
                          paymentListDetails(
                              'model'.tr, "${ctrl.orderData?.model}"),
                          paymentListDetails('chassisNumber'.tr,
                              "${ctrl.orderData?.chassisNumber}"),
                          paymentListDetails(
                              'date'.tr, ctrl.orderData.date ?? ''),
                          Divider(),
                          paymentListDetails('مجموع سعر القطع',
                              "${ctrl.orderData?.items?.fold(0, (previousValue, element) => previousValue + (element.price ?? 0))} ر.س"),
                          paymentListDetails('الخصم',
                              "${ctrl.orderData?.items?.fold(0, (previousValue, element) => previousValue + (element.discount ?? 0))} ر.س"),
                          InkWell(
                            onTap: () async {
                                ...
                                children: [
                                  Row(
                                    children: [
                                      Text("الضريبة",
                                          style: TextStyle(
                                              color: Color(0xFF6B7CAA))),
                                      // SizedBox(width: 5),
                                      //  Icon(Icons.mode_edit, size: 15.0,),
                                    ],
                                  ),
                                  Text(
                                      "${ctrl.orderData.tax ?? '0'} ر.س"), //Color(0xFF4A4A4A)
                                ],
                              ),
                            ),
                          ),
                          ctrl.orderData.deliveryPrice != null &&
                                  ctrl.orderData.deliveryPrice > 0
                              ? paymentListDetails('سعر النقل',
                                  "${ctrl.orderData?.deliveryPrice} ر.س")
                              : SizedBox(),
                          Divider(),
                          paymentListDetails('اجمالي السعر',
                              "${Helpers.fixPrice(ctrl.orderData.totalPrice) ?? '-'} ر.س",
                              size: 21.0),
                          _userAuth.uid == ctrl.orderData?.shopId &&
                                  ctrl.orderData.deliveryPrice != null &&
                                  ctrl.orderData.deliveryPrice > 0
                              ? paymentListDetails('سعر النقل',
                                  "${ctrl.orderData.deliveryPrice}-",
                                  size: 17.0, clr: Get.theme.errorColor)
                              : SizedBox.shrink(),
                          paymentListDetails(
                              '',
                              ctrl.orderData.status > 4 && ctrl.orderData.payed
                                  ? "تم الدفع"
                                  : "",
                              size: 17.0,
                              clr: Color(0xFF34B55E)),
                        ],
                      )
                    : SizedBox.shrink(),
              ),
              _userAuth.uid == ctrl.orderData?.shopId &&
                      ctrl.orderData?.status == 1
                  ? flatButtonWidget(
                      context: context,
                      text: 'ارسال العرض',
                  ...
                  : SizedBox.shrink(),
              _userAuth.uid == ctrl.orderData?.userId &&
                      ctrl.orderData?.status == 2
                  ? flatButtonWidget(
                      context: context,
                      text: 'قبول العرض',
                  ...
                  : SizedBox.shrink(),
              _userAuth.uid == ctrl.orderData?.userId &&
                      ctrl.orderData?.status == 4
                  ? flatButtonWidget(
                      context: context,
                      text: 'تحديد موقع الاستلام',
                      callFunction: () async {
                        Get.put(OrderingController());
                        Get.to(SelectLocation(isSpareOrder: true, spare: {
                          "shopName": ctrl.orderData.shopName,
                          "shopPhone": ctrl.orderData.shopPhone,
                          "fromLat": ctrl.orderData.shopAddressData["lat"],
                          "fromLng": ctrl.orderData.shopAddressData["lng"],
                          "id": ctrl.orderData.id
                        }));
                      },
                    )
                  : SizedBox.shrink(),
              _userAuth.uid == ctrl.orderData?.userId &&
                      ctrl.orderData.status >= 5 &&
                      ctrl.orderData?.payed == false
                  ? flatButtonWidget(
                      context: context,
                      text: 'التوجه لدفع',
                  ...
                  : SizedBox.shrink(),
              _userAuth.uid == ctrl.orderData?.shopId &&
                      ctrl.orderData?.driverId != null &&
                      ctrl.orderData.status == 6 &&
                      ctrl.orderData.deliveryPrice != null &&
                      ctrl.orderData.deliveryPrice > 0
                  ? flatButtonWidget(
                      context: context,
              ...
              _userAuth.uid == ctrl.orderData?.shopId &&
                  ...
                  : SizedBox.shrink(),
              ctrl.orderData.status == 9 &&
                      ctrl.orderData.shopVoted != true &&
                      FirebaseAuth.instance.currentUser.uid ==
                          ctrl.orderData.userId
                  ? flatButtonWidget(
                      context: context,
                      text: 'ratingSHOP'.tr,
                      callFunction: () async {
                        var x = await Get.to(RatingScreen(
                            type: 'shop',
                            type2: 'spares',
                            userID: ctrl.orderData.shopId,
                            voter: controller.user,
                            orderID: ctrl.orderData.id));
                        if (x == "voted") {
                          Get.back();
                        }
                      })
                  : SizedBox.shrink(),
              ctrl.orderData.status == 9 &&
                      ctrl.orderData.clientRated != true &&
                      FirebaseAuth.instance.currentUser.uid ==
                          ctrl.orderData.shopId
                  ? flatButtonWidget(
                      context: context,
  ...
  Widget paymentMethodCard(
      {IconData icon, String name, bool selected = false, String key}) {
    return Container(
...
class _PickupTypeState extends State<PickupType> {
  final Map<int, Widget> pickupTypeWidget = <int, Widget>{
    0: Padding(
        ...
        child: Text('استلام من المحل')),
    1: Padding(
        padding: EdgeInsets.only(top: 5.0, bottom: 7.0),
        child: Text('طلب توصيل')),
  };
                  ...
                  Container(
                    ...
                    child: CupertinoSegmentedControl(
                      ...
                      onValueChanged: (int val) {
                        ...
                        ctrl.orderData.pickupTypeString =
                            val == 0 ? 'استلام من المحل' : 'طلب توصيل';
