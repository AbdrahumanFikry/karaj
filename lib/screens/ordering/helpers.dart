...
class Helpers {
  ...
  static Future<List<LatLng>> decodeEndpointPolyline(String endpoint) async {
    ...
    if (endpoint != null) {
      ...
      List<PointLatLng> decodePolyLinePointsResult =
          polylinePoints.decodePolyline(endpoint);
  ...
  static Future<bool> checkLocationPermission() async {
    ...
    if (!gpsEnabled) {
      GetxWidgetHelpers.mSnackBar('خدمات تحديد الموقع غير مفعلة',
          'يرجى تفعيل خاصية تحديد الموقع بهاتفك لكي تتمكن من استهدام الخاصية.',
    ...
    if (permission == null) {
      GetxWidgetHelpers.mSnackBar('خدمات تحديد الموقع غير مفعلة',
          'تم رفض السماح باستخدام خاصية تحديد الموقع.',
          duration: Duration(seconds: 3));
      return false;
    }
    ...
    if (permission == LocationPermission.deniedForever) {
      GetxWidgetHelpers.mSnackBar('خدمات تحديد الموقع مرفوضة',
          'خدمة تحديد الموقع مرفضة بشكل دائم لذلك يرجى تفعلها من خصائص التطبيق.',
          duration: Duration(seconds: 3));
      return false;
    }
  ...
  static Future<void> launchUrl({String url}) async {
    if (await canLaunch(url)) {
      ...
      GetxWidgetHelpers.mSnackBar("حدث خطاء!", "لايمكن فتح رابط.");
    }
  }
...
class GetxWidgetHelpers {
  static SnackbarController mSnackBar(String title, String body,
      {Duration duration, Color colorText}) {
