            ...
            GetBuilder<OrderingController>(
              builder: (controller) => Column(
                children: [
                  paymentListDetails(
                      'openCounter'.tr,
                      "${controller.order.distanceStartPrice} ${"sar".tr}" ??
                          ''),
                  paymentListDetails(
                      'distance'.tr,
                      "${controller.order.distance.toString()} ${"km".tr}" ??
                          ''),
                  paymentListDetails('distancePrice'.tr,
                      "${controller.order.distancePrice}  ${"sar".tr}" ?? ''),
                  paymentListDetails('vehiclePrice'.tr,
                      "${controller.order.vehiclePrice}  ${"sar".tr}" ?? ''),
                  controller.order.orderType == 2
                      ? Column(
                          children: [
                            paymentListDetails(
                                controller.order.fromFloorString ?? '',
                                "${controller.order.fromFloorPrice}  ${"sar".tr}" ??
                                    ''),
                            paymentListDetails(
                                controller.order.toFloorString ?? '',
                                "${controller.order.toFloorPrice}  ${"sar".tr}" ??
                                    ''),
                          ],
                        )
                      : SizedBox.shrink(),
                  Divider(),
                  paymentListDetails('totalPrice'.tr,
                      "${controller.order.totalPrice ?? ''}SAR",
                      size: 21.0)
                ],
              ),
            ),
            SizedBox(height: Get.height * 0.03),
            controller.order.isSpareOrder == true
                ? Container(
                    width: Get.width * 0.9,
                    child: flatButtonWidget(
                        ...
                        callFunction: () {
                          ...
                          double deliveryPrice =
                              controller.order.totalPrice ?? 0.0;
                          double sparesTotalPrice =
                              Get.find<ModifiedOrderController>()
                                  .orderData
                                  ?.offerPrice;
                          Get.find<ModifiedOrderController>()
                              .orderData
                          ...
                          Get.find<ModifiedOrderController>()
                                  .orderData
                                  ?.totalPrice =
                              (sparesTotalPrice ?? 0) + (deliveryPrice ?? 0);
                          Get.find<ModifiedOrderController>()
                              .orderData
                              ?.status = 5;
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              paymentMethodCard(
                                  ...
                                  name: "card".tr + "-" + "madaCard".tr,
                                  key: "CARD"),
                              // paymentMethodCard(icon: Icons.money_sharp, name: "madaCard".tr, key: "MADA"),
                              paymentMethodCard(
                        ...
                        child: flatButtonWidget(
                            ...
                            callFunction: () {
                              if (controller.order.payVia == null) {
                                GetxWidgetHelpers.mSnackBar("paymentMethod".tr,
                                    "selectPaymentMethod".tr);
                                return;
                              }
                              Get.to(() => PayOrderScreen(
                                  paymentMethod: controller.order.payVia));
                            }),
                      ),
                    ],
                  ),
