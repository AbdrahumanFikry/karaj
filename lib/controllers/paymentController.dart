...
class PaymentController extends GetxController {
  ...
  static Future<Map<String, dynamic>> sendAnInitialPayment({Map<String, dynamic> data, String payVia, String orderType}) async {

    if(data != null) {
      try {
        String url = "https://eu-prod.oppwa.com/v1/payments?";
        String entityId = payVia == "MADA" ? "8ac7a4c97bdf4034017bdf51a47f0048" : "8ac7a4c97bdf4034017bdf501db60042";
        ...
        http.Response res = await http.post(
            ...
            headers: <String, String>{
              ...
              'Authorization': 'Bearer OGFjN2E0Yzk3YmRmNDAzNDAxN2JkZjRmMGQ2YjAwM2V8WjR6aFlqRlhmZQ=='
            },
        ...
        if(res.statusCode == 200) {
          if(body != null && res.body.contains("redirect")) {
            http.Response resVerifiedCode = await http.post(
              Uri.tryParse(body["redirect"]["url"]),
              headers: <String, String>{
                'Content-Type': 'application/json; charset=UTF-8',
                'Authorization': 'Bearer OGFjN2E0Yzk3YmRmNDAzNDAxN2JkZjRmMGQ2YjAwM2V8WjR6aFlqRlhmZQ=='
              },
              body: jsonEncode({
                body["redirect"]["parameters"][0]["name"]: body["redirect"]["parameters"][0]["value"],
                body["redirect"]["parameters"][1]["name"]: body["redirect"]["parameters"][1]["value"],
              }),
            );

            String urlForm = body["redirect"]["url"];

            if(urlForm.endsWith('/')) {
              urlForm = urlForm.substring(0, urlForm.length - 1);
            }

            urlForm = "$urlForm?${body["redirect"]["parameters"][0]["name"]}=${body["redirect"]["parameters"][0]["value"]}";
            urlForm = "$urlForm&${body["redirect"]["parameters"][1]["name"]}=${body["redirect"]["parameters"][1]["value"]}";

            if(res.body.contains("id")) {
              if(orderType == 'spares') {
                Get.find<ModifiedOrderController>().order.value.paymentID = body["id"];
              } else {
                Get.find<OrderingController>().order.paymentId = body["id"];
              }

            }

            return {"status": "pending", "paymentId": "","url": urlForm};
          }
          ...
          if(body != null && res.body.contains("result")) {
            ...
            if(result != null && result.containsKey("code")) {
              if(result["code"].toString() == "000.100.110") {
                ...
                return {"status": "payed", "paymentId": body["ndc"]};
              }
            }
          }
          return data;
          ...
          if(body != null && res.body.contains("result")) {
           Map<String, dynamic> result = body["result"];
            ...
            Get.snackbar("حدث خطاء", result["description"]);
          }
          return  {"status": "error", "paymentId": "123"};
        }
      } catch (e) {
  ...
  static Future<Map<String, dynamic>> checkPaymentStatus({String payVia, String paymentId}) async {
      print("checkPaymentStatus $paymentId");
      try {
        String url = "https://eu-prod.oppwa.com/v1/payments/$paymentId?";
        String entityId = payVia == "MADA" ? "8ac7a4c97bdf4034017bdf51a47f0048" : "8ac7a4c97bdf4034017bdf501db60042";
        url += "entityId=$entityId";
        ...
        if(body != null && res.body.contains("result")) {
          ...
          if(result != null && result.containsKey("code")) {
            if(result["code"].toString() == "000.100.110") {
              ...
              return {"status": "error", "message": result["description"] ?? "حدث خطاء"};
            }
          }
        }
        return {"status": "error", "message": body["result"]["description"] ?? "حدث خطاء"};
      } catch (e) {
        Get.snackbar('حدث خطاء!', e.toString());
        return {"status": "error", "paymentId": "123"};
      }
  ...
  static Future createOrderPayment({String title, OrderModel order, String paymentID, String payVia, String type = "order", String typeX, String forId, bool cutCommission = false}) async {
  ...
  static Future createCommissionPayment({OrderModel order, String userId, String title}) async {
  ...
  static Future createSparePayment({String title, SparesModel order, String paymentID, String payVia, String type = "spares", String typeX, String forId, bool cut = false}) async {
