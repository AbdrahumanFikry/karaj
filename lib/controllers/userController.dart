...
class UserController extends GetxController {
  ...
  void streamingUserData() {
    ...
    if (FirebaseAuth.instance.currentUser != null &&
        _userModel.value.id == FirebaseAuth.instance.currentUser?.uid) {
      _userModel.bindStream(Database().streamGetUser());
      print(
          "================================== bindStream user Data has been started");
    }
  }
  ...
  Future<void> getUserData({bool isOnInit = false}) async {
    if (FirebaseAuth.instance.currentUser != null &&
      ...
      print(
          "================================== Start getting UserData from userController");
      UserModel _user =
  ...
  Future<void> setDriverOnline(bool val) async {
    ...
    if (val == true) {
      print(
          "================================== setDriverOnline() => await Helpers.checkLocationPermission");
      if (await Helpers.checkLocationPermission()) {
        print(
            "================================== setDriverOnline() => await Helpers.checkLocationPermission CHECKED TRUE");
        ...
        GeoFirePoint myLocation =
            geo.point(latitude: p.latitude, longitude: p.longitude);
        bool vanCar = false;
        bool pickupCar = false;
        List vehicles = _userModel.value.personalData != null
            ? _userModel.value.personalData['vehicles'] ?? []
            : [];
        ...
        _firestore.collection('availableDrivers').doc(_userModel.value.id).set({
          'name': _userModel.value.firstName,
          'uid': _userModel.value.id,
          'vanCar': vanCar,
          'pickupCar': pickupCar,
          'token': _userModel.value.token,
          'position': myLocation.data
        });
      } else {
        print(
            "================================== setDriverOnline() => await Helpers.checkLocationPermission UNCHECKED FALSE");
  ...
  Future<void> submitOrder(OrderModel order) async {
    ...
    await Database().updateDocument(
        docName: "users",
        docID: order.userId,
        data: {"isOnRide": true, "onOrderID": order.id});
    final geo = Geoflutterfire();
    GeoFirePoint center =
        geo.point(latitude: order.fromLat, longitude: order.fromLng);
    ...
    stream.listen((List<DocumentSnapshot> documentList) {
      ...
      if (!stopStreaming) {
        documentList.forEach((element) {
          print("===========================================");
          if (!tokensSended.contains((element.data() as Map)['token'] ?? '')) {
            if (order.vehicleType == 1 && element["vanCar"] == false) {
            } else if (order.vehicleType == 2 &&
                element["pickupCar"] == false) {
            } else {
              print(
          ...
          tokensSended.add((element.data() as Map)['token'] ?? '');
